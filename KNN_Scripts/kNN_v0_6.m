% KNN clustering on statistical sub-features extracted from CM EMI data using Descriptive stats.
% Added centroid (Median) based kNN approach to reduce misclassification
% Manoj Gulati
% IIITD
% DOM:16-09-2015
...
feat_len2 = [1 4 1 4 1 1 1 1 1 4]; % CM Mode EMI
% feat_len2 = [1 1 1 1 1 1 4 1 1 1]; % DM Mode EMI
...
Path3 = strcat('DIM',int2str(dim),'_Centroid_Mean_RD/');
...
%% Compute centroid from training data matrix
...
% centroid = [];
% for index=(1:9)
%     centroid(index) = median(LC1(index,:));
% end
%% % Data for KNN training
X = [BGN_CT'; LC1_CT'; LCD1_CT'; CFL1_CT'; CPU1_CT'; PRT1_CT'];
% % Labels for KNN training
Y = [ones(1,1); 2*ones(1,1); 3*ones(1,1); 4*ones(1,1); 5*ones(1,1); 6*ones(1,1);];

% disp('Starting learning phase using KNN');

% mdl = ClassificationKNN.fit(X,Y,'NumNeighbors',1,'BreakTies','random');    
